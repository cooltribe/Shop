...
public class JsonToProductList {
	/**
	 * 解析json数据，得到具体的实体类
	 * 
	 * @param json
	 ...
	 */
	public static  PdaResponse<List<ProductDto>> parserLoginJson(String json)
			throws JSONException {
		PdaResponse<List<ProductDto>> response = new PdaResponse<List<ProductDto>>();
		try {
			Type type = new TypeToken<PdaResponse<List<ProductDto>>>() {
			}.getType();
