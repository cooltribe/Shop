...
public class WriteJudgeActivity1 extends Activity implements OnClickListener {
	...
	private static WriteJudgeActivity1 instance;
	...
	private void init() {
		...
		options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.mrpic_little).showImageForEmptyUri(R.drawable.mrpic_little)
	...
	private void findView() throws Exception {
		...
		View include = findViewById(R.id.include1);
		View view = View.inflate(instance, R.layout.judge_image, null);
	...
	private void showOptionDialog(final int photoCode, final int cameraCode) {
		final SelectPicPopupWindow dialog = new SelectPicPopupWindow(WriteJudgeActivity1.this);
		...
		dialog.setSecendButtonOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				CommonUtils.selectSystemPhone(photoCode, WriteJudgeActivity1.this);
				dialog.dismiss();
			}
		});
		...
		// 显示窗口
		dialog.showAtLocation(WriteJudgeActivity1.this.findViewById(R.id.main), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0); // 设置layout在PopupWindow中显示的位置
	}
